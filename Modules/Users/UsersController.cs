...
namespace RaaiVan.Modules.Users
{
    public static class UsersController
    {
        ...
        {
            return USRParsers.last_passwords(DBConnector.read(null, GetFullyQualifiedName("GetLastPasswords"),
                userId, autoGenerated, count));
        }

        public static DateTime? get_last_password_date(Guid userId)
        {
            return DBConnector.get_date(null, GetFullyQualifiedName("GetLastPasswordDate"), userId);
        }

        public static void get_current_password(Guid userId, ref string password, ref string passwordSalt)
        {
            DBResultSet results = DBConnector.read(null, GetFullyQualifiedName("GetCurrentPassword"), userId);
            ...
            ref string password, ref string passwordSalt)
        {
            ...
            if(userId.HasValue) get_current_password(userId.Value, ref password, ref passwordSalt);
        }
        ...
        {
            return DBConnector.get_guid(null, GetFullyQualifiedName("GetPassResetTicket"), userId);
        }
        ...
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("SetLastActivityDate"), userId, DateTime.Now);
        }
        ...
        private static User _locked(Guid userId, bool? locked)
        {
            if (locked.HasValue)
            {
                return DBConnector.succeed(null, GetFullyQualifiedName("Locked"), userId, locked, DateTime.Now) ?
                    new User() { IsLockedOut = locked } : null;
            }
            else
                return USRParsers.lockout_date(DBConnector.read(null, GetFullyQualifiedName("Locked"), userId, locked, DateTime.Now));
        }

        public static bool lock_user(Guid userId)
        {
            return _locked(userId, locked: true) != null;
        }

        public static bool lock_user(Guid? applicationId, string username)
        {
            Guid? userId = get_user_id(applicationId, username);
            return userId.HasValue && lock_user(userId.Value);
        }

        public static bool unlock_user(Guid userId)
        {
            return _locked(userId, locked: false) != null;
        }

        public static bool unlock_user(Guid? applicationId, string username)
        {
            Guid? userId = get_user_id(applicationId, username);
            return userId.HasValue && unlock_user(userId.Value);
        }

        public static User locked(Guid userId)
        {
            return _locked(userId, locked: null);
        }

        public static User locked(Guid? applicationId, string username)
        {
            Guid? userId = get_user_id(applicationId, username);
            return !userId.HasValue ? null : locked(userId.Value);
        }

        public static int login_attempt(Guid userId, bool succeed)
        {
            return DBConnector.get_int(null, GetFullyQualifiedName("LoginAttempt"), userId, succeed);
        }

        public static int login_attempt(Guid? applicationId, string username, bool succeed)
        {
            Guid? userId = get_user_id(applicationId, username);
            return !userId.HasValue ? 0 : login_attempt(userId.Value, succeed);
        }
        ...
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("SetPhoneNumber"),
        ...
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("EditPhoneNumber"),
        ...
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("RemovePhoneNumber"), numberId, currentUserId, DateTime.Now);
        }

        public static bool set_main_phone(Guid numberId, Guid userId)
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("SetMainPhone"), numberId, userId);
        }

        public static bool set_email_address(Guid emailId, Guid userId, string address, Guid currentUserId)
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("SetEmailAddress"),
        ...
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("EditEmailAddress"),
                emailId, address, currentUserId, DateTime.Now);
        }

        public static bool remove_email_address(Guid emailId, Guid currentUserId)
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("RemoveEmailAddress"), emailId, currentUserId, DateTime.Now);
        }

        public static bool set_main_email(Guid emailId, Guid userId)
        {
            return DBConnector.succeed(null, GetFullyQualifiedName("SetMainEmail"), emailId, userId);
        }

        public static List<PhoneNumber> get_phone_numbers(Guid userId)
        {
            return USRParsers.phone_numbers(DBConnector.read(null, GetFullyQualifiedName("GetPhoneNumbers"), userId));
        }

        public static PhoneNumber get_phone_number(Guid numberId)
        {
            return USRParsers.phone_numbers(DBConnector.read(null, GetFullyQualifiedName("GetPhoneNumber"), numberId)).FirstOrDefault();
        }

        public static Guid? get_main_phone(Guid userId)
        {
            return DBConnector.get_guid(null, GetFullyQualifiedName("GetMainPhone"), userId);
        }

        public static List<EmailAddress> get_email_addresses(Guid userId)
        {
            return USRParsers.email_addresses(DBConnector.read(null, GetFullyQualifiedName("GetEmailAddresses"), userId));
        }

        public static EmailAddress get_email_address(Guid emailId)
        {
            return USRParsers.email_addresses(DBConnector.read(null, GetFullyQualifiedName("GetEmailAddress"), emailId)).FirstOrDefault();
        }
        ...
        {
            return DBConnector.get_guid(null, GetFullyQualifiedName("GetMainEmail"), userId);
        }

        public static List<PhoneNumber> get_users_main_phone(List<Guid> userIds)
        {
            return USRParsers.phone_numbers(DBConnector.read(null, GetFullyQualifiedName("GetUsersMainPhone"),
                ProviderUtil.list_to_string<Guid>(userIds), ','));
        }

        public static List<EmailAddress> get_users_main_email(List<Guid> userIds)
        {
            return USRParsers.email_addresses(DBConnector.read(null, GetFullyQualifiedName("GetUsersMainEmail"),
                ProviderUtil.list_to_string<Guid>(userIds), ','));
        }
