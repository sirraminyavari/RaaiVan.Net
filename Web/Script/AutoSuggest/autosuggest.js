...
//Ramin's
function my_guid_generator() {
    var S4 = function () {
        return (((1 + Math.random()) * 0x10000) | 0).toString(16).toUpperCase().substring(1);
    }

    return (S4() + S4() + "-" + S4() + "-" + S4() + "-" + S4() + "-" + S4() + S4() + S4());
}

function initialize_autosuggest(autosuggestObject, params) {
    if (autosuggestObject === undefined) return;

    autosuggestObject.limit_start = false;
    autosuggestObject.match_first = true;
    autosuggestObject.item_delimiter = '&cent;';
    autosuggestObject.use_scroll = false;
    autosuggestObject.use_iframe = false;
    autosuggestObject.full_refresh = true;
	
    if (params == null) params = { "!": 0 };
    if (typeof (params["StartCheck"]) != "undefined") autosuggestObject.start_check = params["StartCheck"];

    //set the start check: 0
    autosuggestObject.start_check = 0; // :)

    autosuggestObject.ResponseParser = params.ResponseParser;
    if (params.OnEnter) autosuggestObject.onEnter = params.OnEnter;
    if (params.OnSelect) autosuggestObject.onSelect = params.OnSelect;
    if (params.OnBindURL) autosuggestObject.onBindURL = params.OnBindURL;
}

function add_item_to_autosuggest(autosuggestObject, item, params) {
    var curSelectedIndex = autosuggestObject.selectedIndex;
    var _itemsArray = [];

    for (var i = 0, lnt = autosuggestObject.values.length; i < lnt; ++i)
        _itemsArray.push([autosuggestObject.keywords[i], autosuggestObject.values[i]]);

    _itemsArray.push(item);

    autosuggestObject.values = [];
    autosuggestObject.keywords = [];
    autosuggestObject.bindArray(_itemsArray);
    autosuggestObject.selectedIndex = curSelectedIndex;
}

function modify_autosuggest_item(autosuggestObject, item, params) {
    var curSelectedIndex = autosuggestObject.selectedIndex;
    var _itemsArray = [];

    for (var i = 0, lnt = autosuggestObject.values.length; i < lnt; ++i) {
        var value = autosuggestObject.values[i];
        var keyword = value == item[1] ? item[0] : autosuggestObject.keywords[i];

        _itemsArray.push([keyword, value]);
    }
    ...
    autosuggestObject.bindArray(_itemsArray);
    autosuggestObject.selectedIndex = curSelectedIndex;
    if (curSelectedIndex >= 0 && autosuggestObject.values[curSelectedIndex] == item[1]) autosuggestObject.field.value = item[0];
}

function remove_autosuggest_item(autosuggestObject, itemId, params) {
    var curSelectedIndex = autosuggestObject.selectedIndex;
    var _itemsArray = [];
    ...
    for (var i = 0, lnt = autosuggestObject.values.length; i < lnt; ++i) {
        if (autosuggestObject.values[i] == itemId)
            itemIndex = i;
        else
            _itemsArray.push([autosuggestObject.keywords[i], autosuggestObject.values[i]]);
    }
    ...
    autosuggestObject.keywords = [];
    autosuggestObject.bindArray(_itemsArray);
    autosuggestObject.selectedIndex = (itemIndex >= 0 && itemIndex < curSelectedIndex) ? curSelectedIndex - 1 :
        (itemIndex == curSelectedIndex ? -1 : curSelectedIndex);
    if (itemIndex == curSelectedIndex) autosuggestObject.field.value = "";
}

function clear_autosuggest(autosuggestObject, params) {
    autosuggestObject.clear(params);
};
//end of Ramin's
...
function autosuggest(id, array, url, onSelect) {
    //Ramin's
    var field = typeof (id) == "object" ? id : document.getElementById(id);
    ...
    /////////////////////////////////////////////////////
    //                                                 //
 	var my_new_id = my_guid_generator();               //
    field.setAttribute("id", my_new_id);               //
	id = my_new_id;                                    //
    //                                                 //
    /////////////////////////////////////////////////////
    /////////////////////////////////////////////////////
...
autosuggest.prototype = {
    ...
    bindURL: function (url) {
        if (!url) url = autosuggest_url;
        this.suggest_url = url;
        if (this.onBindURL) this.onBindURL(url);
    },
    ...
    empty: function () {
        ...
        //if (window.jQuery) jQuery(this.field).focus();
    },
    ...
    clear: function (params) {
        ...
        this.values = [];
        this.keywords = [];
        if (params.ClearURL === true) this.suggest_url = "";
    },

    set_item: function (value, keyword, params) {
        params = params || {};

        this.field.value = keyword;
        this.field.style.color = "";
        this.field.style.fontWeight = "";

        var index = -1;
        for (var i = 0, lnt = (this.values || []).length; i < lnt; ++i) {
            if (this.values[i] == value) {
                this.selectedIndex = i;
                if (this.onSelect && !params.IgnoreOnSelect) this.onSelect(i, this);
                return;
            }
        }
        ...
        if (this.onSelect && !params.IgnoreOnSelect) this.onSelect(0, this);
    },
    ...
    set_selected: function (row, selected) {
        row.style.textShadow = selected ? "0 2px 2px rgba(95,95,95,.4)" : "0 0px 0px rgba(0,0,0,0)";
        row.classList[selected ? "add" : "remove"]("SoftBackgroundColor");
    },
    //end of Ramin's
    ...
    build: function () {
        ...
        var bb = GlobalUtilities.create_nested_elements([
            {
                Type: "div", ID: "suggest_table_" + this.field_id, Name: "_div",
                Class: "autosuggest-body rv-border-radius-quarter SoftBorder",
                Style: "position:absolute; z-index:999999; background-color:white;"
            }
        ])["_div"];

        //Ramin's
        var inputWidth = $(this.field).outerWidth();
        bb.style.maxWidth = inputWidth + "px";
        //end of Ramin's

        GlobalUtilities.stick(this.field, bb, { Fit: true });
        ...
        function createArrowRow(dir) {
            table.HASARROWS = true;

            var row = GlobalUtilities.create_nested_elements([
                {
                    Type: "div", Class: "WarmBackgroundColor", Name: "_div",
                    Style: "color:white; text-align:center; height:1.2rem; cursor:pointer;", 
                    Childs: [{ Type: "i", Class: "fa " + (dir ? "fa-chevron-up" : "fa-chevron-down") }]
                }
            ], table)["_div"];

            real_height += row.offsetHeight;

            return row;
        };

        if (!this.use_scroll && showFull)
            createArrowRow(true).style.color = "rgb(200,200,200)";
        ...
        function addSuggestion(index, _first) {
            var row = that.rows[j] = GlobalUtilities.create_nested_elements([
                {
                    Type: "div", Class: "rv-border-radius-quarter Ellipsis SoftBorder", Name: "_div",
                    Style: "padding:0.3rem; border-width:0px; border-top-width:1px;" +
                        "border-color:rgb(200,200,200); cursor:pointer;"
                }
            ], table)["_div"];

            that.set_selected(row, _first || (that.previous == index));

            //Ramin's
            //cell.innerHTML = that.parse(that.keywords[index], plen, re);
            that._set_cell(row, that.parse(that.keywords[index], plen, re));
            //end of Ramin's
            row.setAttribute("pos", j++);
            row.autosuggest = that;

            if (that.use_mouse) {
                that.addEvent(row, "click", that.funcClick);
                row.onmouseover = that.funcHighlight;
            }
        ...
        this.cur_h = real_height + (table.HASARROWS ? -2 : 2);
    ...
    remake: function () {
        ...
        function adjustArrow(obj, which, cond, handler) {
            var r = a.childNodes[k++];
            r.style.color = cond ? "white" : "rgb(200,200,200)";

            if (cond && handler && obj.use_mouse)
                obj.addEvent(r, "click", handler);
        }
        ...
        for (var i = 0; i < kl; i++) {
            if (this.bool[i]) {
                if ((j >= this.range_up) && (j <= this.range_down)) {
                    var r = this.rows[j] = a.childNodes[k++];
                    this.set_selected(r, false);

                    //Ramin's
                    //c.innerHTML = this.parse(this.keywords[i], plen, re);
                    this._set_cell(r, this.parse(this.keywords[i], plen, re));
                    //end of Ramin's
    ...
    fixArrows: function (base) {
        ...
        if (row) row.style.color = at_start ? "rgb(200,200,200)" : "white";

        row = table.childNodes[this.entry_limit + 1];
        if (row) row.style.color = at_end ? "rgb(200,200,200)" : "white";
    },
    ...
    scroll: function (direction, times) {
        ...
        if (!direction && (this.current < 0)) {
            ...
            var t = this.getRow();
            if (t && t.style) this.set_selected(t, false);
        }
        ...
        if (t && t.style) this.set_selected(t, true);
    ...
    mouseClick: function (event) {
        ...
        var elem = event.srcElement || event.target;
        if (String((elem || {}).tagName).toLowerCase() == "span") elem = elem.parentNode;
        
        var obj = elem.autosuggest || (elem.parentNode || {}).autosuggest ||
            ((elem.parentNode || {}).parentNode || {}).autosuggest;
    ...
    highlightTable: function (event) {
        ...
        if (row && row.style) obj.set_selected(row, false);

        obj.current = parseInt(elem.getAttribute("pos"), 10);

        row = obj.getRow();

        if (row && row.style) obj.set_selected(row, true);
    ...
    choose: function () {
        ...
        for (var i = 0, c = 0; i < kl; i++)
            if (this.bool[i] && (c++ == this.current)) break;

        this.selectedIndex = i;
        this.insertWord(this.keywords[i]);

        if (this.onSelect) this.onSelect(i, this);
    },
    ...
    keyPress: function (event) {
        ...
        if (obj.restrict_typing && !obj.suggest_url.length &&
            (GlobalUtilities.get_type(obj.suggest_url) != "function") && (code >= 32)) {
    ...
    checkKey: function (event) {
        ...
        switch (code) {
            ...
            case 13:
                //Ramin's
                try { __MASTER_FORM.onsubmit = __MASTER_FORM_RETURN_FALSE; }
                catch (e) { }

                if (code == 13 && obj.onEnter && (obj.onEnter(event) === false)) {
                    obj.hide();
                    return false;
                }
                //end of Ramin's
    ...
    preSuggest: function (kc) {
        ...
        if (this.text_delimiter.length > 0) {
            ...
            //Ramin's
            var __val = GlobalUtilities.verify_string(this.field.value);
            //end of Ramin's

            for (var i = 0, j = __val.length; i < __val.length; i++, j--) {
                if (__val.substr(i, j).search(delim_split_rx) == 0) {
                    var ma = __val.substr(i, j).match(delim_split_rx);
        ...
        if (ot.length == 0 && (kc != -1)) {
            ...
            this.hide();
		       ...
		       ((ot.length > 1 /* Ramin's */ || ot.length >= this.start_check /* end of Ramin's */) && !this.keywords.length) ||
            ...
            if (GlobalUtilities.get_type(this.suggest_url) == "function") {
                //Ramin's
                if (t.length < this.start_check) return;
                //end of Ramin's

                this.suggest_url(GlobalUtilities.verify_string(t), function (arrResponse) {
                    var __keys = [], __vals = [];

                    for (var i = 0, lnt = (arrResponse || []).length; i < lnt; ++i) {
                        __keys.push(arrResponse[i][0]);
                        __vals.push(arrResponse[i][1]);
                    }

                    that.keywords = that.keywords_init = __keys;
                    that.values = that.values_init = __vals;

                    that.suggest(ot_, t);
                });
            }
            else if (this.suggest_url.length) {
                if (t.length < this.start_check) return;

                var _reqUrl = this.suggest_url +
                    Base64.encode(GlobalUtilities.verify_string(t)) + "&rndts=" + new Date().getTime();

                RVRequest.post_request(_reqUrl, null, function (text) {
                    if (!that.ResponseParser) return;

                    var _did = false;

                    var _done = function (results) {
                        if (_did) return;
                        _did = true;
                        ...
                        for (var i = 0, lnt = results.length; i < lnt; ++i) {
                            __keys.push(results[i][0]);
                            __vals.push(results[i][1]);
                        }
                    ...
                    var _parsedResult = that.ResponseParser(text, function (r) { _done(r) }) || [];
                    if (_parsedResult.length > 0) _done(_parsedResult);
                }, null, false);
            }
    ...
    suggest: function (ot, t) {
        ...
        t = GlobalUtilities.verify_string(t);
        //end of Ramin's
